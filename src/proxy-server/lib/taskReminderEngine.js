...
/**
 * Task reminder engine (Railway-native) — runs every 15 min via Railway Cron.
 *
 * Behavior:
 *   - selects incomplete tasks whose due_date falls within the next 15-minute
 *     window (0 <= due - now <= 15min).
 ...
 *     does not abort the run (best-effort).
 *
 * Data source: Railway tasks table (Postgres). No Base44 dependency.
...
async function listDueSoonTasks() {
  const allTasks = await rda.list('Task', '-created_at', 5000, 0).catch(() => []);
  const now = Date.now();
  return (allTasks || []).filter(task => {
    // Railway tasks table: incomplete = no completed_at AND status != 'completed'
...
async function processTaskReminders({ dryRun = false, triggeredBy = 'manual' } = {}) {
  ...
  for (const task of dueSoon) {
    let lead = null;
    if (task.lead_id) {
      try { lead = await rda.get('Lead', task.lead_id).catch(() => null); } catch (_) {}
    }
    ...
    try {
      await emailService.send({
        ...
        htmlBody: taskBody({ title: task.title, dueDate, dueTime, leadName: leadName || '', notes: taskNotes }),
