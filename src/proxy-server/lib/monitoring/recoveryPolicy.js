...
// Circuit breaker: services managed by dedicated cron/loop workers.
// Restarting these via Railway API could create duplicate execution.
// Alert only — operator must investigate and restart manually.
const CIRCUIT_BREAKER_SERVICES = ['reminder-worker', 'calendar-outbox-worker'];

function evaluateRecovery(serviceId, failureInfo) {
  const { errorSummary, isCrashLoop, isNewIncident, previousHealthy } = failureInfo;

  // Circuit breaker — never auto-restart cron/loop-managed workers
  if (CIRCUIT_BREAKER_SERVICES.includes(serviceId)) {
    return {
      action: 'escalate',
      reason: `Circuit breaker: ${serviceId} is managed by a dedicated cron/loop worker. Auto-restart blocked to prevent duplicate execution. Operator must investigate and restart manually.`,
