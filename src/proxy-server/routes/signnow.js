...
router.post('/by-external/:externalRef/prepare', requireAdminManager, async (req, res) => {
  try {
    ...
    // Construct the SignNow editor URL so the user can open the document
    const editorUrl = `https://app.signnow.com/webapp/document/${docId}`;

    // Store in Postgres (signing_url = editor URL for template-prepared docs)
    const ins = await query(
      `INSERT INTO signnow_documents (lead_id, document_id, template_id, document_name, status, signers, signing_url, created_by)
    ...
    // Write a CRM activity note (matches the old Base44 behavior)
    const signerNote = inviteSent && lead.email ? ` — sent to ${lead.email}` : '';
    await query(
      `INSERT INTO activities (lead_id, type, content, author, source, created_at)
       VALUES ($1, 'note', $2, $3, 'manual', NOW())`,
      [lead.id, `📄 SignNow document created from template: "${ins.rows[0].document_name}"${signerNote}`, req.user.email]
    ).catch(e => console.warn('[signnow] activity log failed:', e.message));
