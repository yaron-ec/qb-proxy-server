...
router.put('/by-external/:externalRef', requireAuth, async (req, res) => {
  try {
    ...
    if (!insertFirstName || !insertLastName) {
      const { whereSql: existWhere } = leadIdWhere(externalRef);
      const existing = await query(`SELECT first_name, last_name FROM leads WHERE ${existWhere}`, [externalRef]);
      if (existing.rows[0]) {
    ...
    const { whereSql: upsertWhere } = leadIdWhere(externalRef);
    const existingById = await query(`SELECT id, external_ref FROM leads WHERE ${upsertWhere} LIMIT 1`, [externalRef]);
...
router.delete('/by-external/:externalRef', requireAuth, async (req, res) => {
  try {
    ...
    const { whereSql: delWhere } = leadIdWhere(externalRef);
    const leadR = await query(`SELECT id, external_ref, owner_id FROM leads WHERE ${delWhere}`, [externalRef]);
