...
router.get('/:id/financials', async (req, res) => {
  try {
    ...
    // Guard: crm_sale_id is a UUID column. A non-UUID legacy identifier would
    // cause PostgreSQL "invalid input syntax for type uuid" crash.
    if (!UUID_RE.test(String(saleId))) {
      return res.status(400).json({ error: 'invalid_id', message: 'sale_id must be a valid Railway UUID' });
    }
