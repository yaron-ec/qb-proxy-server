...
/**
 * computeCompletionFields — pure decision for whether an update/create
 * should stamp completed_at/completed_by. Set EXACTLY ONCE, the first time
 * stage becomes Job Completed while completed_at is not already set — never
 * overwritten afterward, so it stays a true historical fact even if the
 * deal is edited again later or briefly reopened (stage moved away and back).
 * completed_at/completed_by are deliberately absent from DEAL_WRITABLE_FIELDS
 * so a client payload can never set them directly.
 *
 * @param {string|undefined} previousStage - the deal's stage before this update (undefined on create)
 * @param {string|undefined} nextStage - the stage this update/create would set
 * @param {string|null} existingCompletedAt - the deal's current completed_at (null on create)
 * @param {string} actor - email/id of the user performing the action
 * @returns {{completed_at: string, completed_by: string}|null}
 */
function computeCompletionFields(previousStage, nextStage, existingCompletedAt, actor) {
  if (nextStage !== JOB_COMPLETED_STAGE) return null;
  if (existingCompletedAt) return null;
  if (previousStage === JOB_COMPLETED_STAGE) return null; // already completed, no stage change
  return { completed_at: new Date().toISOString(), completed_by: actor || null };
}
...
module.exports = {
  ...
  DEAL_WRITABLE_FIELDS, JOB_OWNED_FIELDS, UUID_RE, isValidUUID, JOB_COMPLETED_STAGE,
  serializeDeal, resolveDealScope, repMatchCandidates, canAccessDeal, canWriteDeal,
  validateDealPayload, computePaymentStatus, applyDealUpdateGuard, computeCompletionFields,
