...
// The logo image on these pages (lib/reminderPages.js) is served from the
// CRM's own public origin, not Base44 — this file's own header already
// claims "Zero Base44"; the CSP img-src below previously still allowlisted
// media.base44.com, which was stale relative to that claim.
const CRM_PUBLIC_ORIGIN = process.env.CRM_PUBLIC_URL || 'https://crm.ecconstructiongroup.com';
...
function setHeaders(res) {
  ...
  res.set('Content-Security-Policy', `default-src 'none'; style-src 'unsafe-inline'; img-src ${CRM_PUBLIC_ORIGIN}; frame-ancestors 'none'; base-uri 'none'`);
}
...
async function currentFingerprint(lead, leadId) {
  if (!lead) return null;
  const p = time.appointmentParts(lead) || { date: null, time: null, type: null };
