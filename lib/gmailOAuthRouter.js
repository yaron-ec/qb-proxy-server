...
// gmail.readonly added to support the lead-scoped correspondence read path
// (routes/leadEmails.js) — production verification confirmed the prior
// send-only grant fails Gmail reads with a genuine Google 403 ("insufficient
// authentication scopes"). Minimum required scope, not gmail.modify (no
// write/delete capability is needed for a read-only correspondence view).
const SCOPES = 'https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/gmail.readonly openid email';
  ...
  async function handleCallback(req, res) {
    try {
      ...
      await credStore.saveGmailCredential(ENVIRONMENT, {
        ...
        // Google's token response includes the actually-granted scope list
        // (space-separated) — stored so /status can truthfully distinguish
        // "connected, send-only" from "connected, read available" without
        // ever exposing the token itself.
        scope: tokenResult.data.scope || null,
      });
  ...
  async function handleStatus(req, res) {
    try {
      ...
      if (cred && cred.refresh_token) {
        // Distinguish "connected, send-only" from "connected, read
        // available" from the stored scope list alone — never the token
        // itself, and never a live Google call from a status check.
        const grantedScopes = String(cred.scope || '').split(/\s+/).filter(Boolean);
        return res.status(200).json({
          connected: true,
          account: EXPECTED_EMAIL,
          environment: ENVIRONMENT,
          status: 'connected',
          has_send_access: grantedScopes.includes('https://www.googleapis.com/auth/gmail.send'),
          has_read_access: grantedScopes.includes('https://www.googleapis.com/auth/gmail.readonly'),
          // A credential saved before this field existed has no recorded
          // scope at all — distinct from "recorded and missing" so an old
          // connection doesn't misreport as definitively send-only.
          scope_recorded: grantedScopes.length > 0,
        });
      }
    } catch (e) {
...
// Exposed for routes/adminGmailOAuth.js — an ADMIN-AUTHENTICATED (Railway
// JWT) bridge into this internal (PROXY_SECRET/setup_token-gated) flow, so
// an admin can reconnect Gmail from Settings without Railway/env access.
// signSetupToken() only mints a short-lived, single-purpose token (can only
// call /start); handleStatus() exposes no token material. Neither widens
// this router's own auth model.
module.exports.signSetupToken = defaultInstance.signSetupToken;
module.exports.handleStatus = defaultInstance.handleStatus;
module.exports.ENVIRONMENT = defaultInstance.ENVIRONMENT;
module.exports.EXPECTED_EMAIL = defaultInstance.EXPECTED_EMAIL;