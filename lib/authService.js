...
async function findOrCreateByGoogleSub(googleSub, email, fullName) {
  ...
  // Check UserAllowlist for the correct role (temporary until all users are
  // provisioned in Railway). Falls back to 'user' if not in the allowlist.
  // This preserves admin/manager/sales_rep roles for known EC Construction
  // Group users on their first Google SSO login, without creating duplicates.
  let role = 'user';
  try {
    // Railway-native: query the user_allowlist table directly (created by
    // migration 2026-14). If the table doesn't exist yet, the query fails
    // and we default to 'user' — same behavior as the old Base44 fallback.
    const { rows } = await db.query(
      `SELECT role FROM user_allowlist WHERE lower(email) = lower($1) AND enabled = true LIMIT 1`,
      [email]
    );
    if (rows.length > 0 && rows[0].role) {
      role = rows[0].role;
    }
  } catch (e) { /* best-effort — default to 'user' */ }

  const ins = await db.query(
    `INSERT INTO users (email, full_name, role, google_sub) VALUES ($1, $2, $3, $4) RETURNING *`,
    [email, fullName || null, role, googleSub]
  );
  return ins.rows[0];
}
