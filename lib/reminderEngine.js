...
async function processReminders({ dryRun = false, triggeredBy = 'manual' } = {}) {
  ...
  // Synthetic test path: dry-run against the test_leads table (REMINDER_TEST_MODE).
  // Gmail is not configured and not needed here, so we skip ALL Gmail logic —
  // no token refresh, no lock check, no credential alert — and record a neutral
  // health state. lib/gmailSender.js is never imported in this path.
  const syntheticTest = dryRun && source === 'postgres' && testMode;

  // Global Gmail credential lock — if set, do not send anything.
  // Skipped on the synthetic test path (Gmail is not involved there).
  if (!syntheticTest) {
  ...
  if (dryRun) {
    let gmailStatus;
    if (syntheticTest) {
      // Synthetic test path: no Gmail config, no auth attempt, no lock.
