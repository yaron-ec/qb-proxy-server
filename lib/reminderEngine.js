...
/**
 * Reminder engine — the core of the Railway-native reminder system.
 *
 ...
 *   5. Write the REMINDER_SENT Activity to Railway Postgres (best-effort,
 *      queued for retry on failure). This is for CRM visibility ONLY and
 *      is never the send gate.
 *
 * REMINDER_DRY_RUN defaults to 'true'. In dry-run the engine computes and
 * logs eligible reminders but makes NO Postgres claim writes and sends NO
 * emails. Set REMINDER_DRY_RUN=false to enable real sending.
 */
'use strict';
...
// Public base URL for the /r/* customer-action pages (opaque-token links in emails).
...
// Structured, secret-free delivery log. Never logs tokens/credentials.
function logReminderSend({ role, recipient, cc, assignedRep, sender, replyTo, subject, gmailMessageId, success, window, error }) {
...
function windowLabel(key) {
  return { '12h': '12 hours', '2h': '2 hours', '30min': '30 minutes' }[key] || key;
}
...
// Customer reminders are keyed to the REAL appointment start (the canonical
// appointments row, projected into reminder_leads.appointment_*), never to the
// travel buffer and never to the lead's follow-up. See time.appointmentParts.
function getAppointmentMs(lead) {
  const p = time.appointmentParts(lead);
  if (!p) return null;
  const utcMs = time.pacificToUtcMs(p.date, p.time);
  return { ms: utcMs, date: p.date, time: p.time, type: p.type };
}
...
// Zero recipients reached (no customer email + no staff window, or customer opted out + no staff).
// Terminal state — prevents re-claim hammering without falsely reporting "sent".
async function markSkippedNoRecipient(claimId) {
  await db.query(
    `UPDATE reminder_claims
     SET status = 'skipped_no_recipient', sent_at = NOW(), gmail_message_ids = '[]'::jsonb,
...
async function sendWindowEmails(lead, win, appt, gmailToken, reminderKey) {
  const gmail = require('./gmailSender'); // lazy: real-send path only (GmailCredentialsError)
  const emailService = require('./emailService'); // lazy: real-send path only (transport + idempotency)
  ...
  const assignedRep = lead.assigned_rep || null;
  const sender = YARON_EMAIL;                                                   // production sender (policy: yaron@ecconstructiongroup.com)
  const fromName = COMPANY_FROM_NAME;
  ...
  const customerSkippedNoEmail = !lead.email && !lead.customer_reminders_disabled;
  ...
  if (sendCustomer) {
    ...
    try {
      const r = await emailService.send({
        to: lead.email, subject, htmlBody: body,
        fromName, fromAddress: sender,
        idempotencyKey: `scheduled-reminder:${lead.id}:${win.key}:${appt.date}:customer:${lead.email}`,
      });
      messageIds.push(r.gmailMessageId);
      logReminderSend({ role: 'customer', recipient: lead.email, cc: [], assignedRep, sender, subject, gmailMessageId: r.gmailMessageId, success: true, window: win.key });
    } catch (e) {
      ...
      logReminderSend({ role: 'customer', recipient: lead.email, cc: [], assignedRep, sender, subject, success: false, window: win.key, error: e.message });
    }
  }
    ...
    for (const recipient of staffRecipients) {
      try {
        const r = await emailService.send({
          ...
          fromName, fromAddress: sender,
          idempotencyKey: `scheduled-reminder:${lead.id}:${win.key}:${appt.date}:staff:${recipient}`,
        });
        messageIds.push(r.gmailMessageId);
        logReminderSend({ role: 'staff', recipient, cc: [], assignedRep, sender, subject, gmailMessageId: r.gmailMessageId, success: true, window: win.key });
      } catch (e) {
        if (e instanceof gmail.GmailCredentialsError) return { messageIds, hadFailure: true, credentialError: e.message, lastError: e.message, errorType: 'gmail_credentials' };
        hadFailure = true; lastError = e.message; errorType = 'gmail_send';
        logReminderSend({ role: 'staff', recipient, cc: [], assignedRep, sender, subject, success: false, window: win.key, error: e.message });
      }
    }
  }

  return { messageIds, hadFailure, credentialError, lastError, errorType, customerSkippedNoEmail, sendStaff, sendCustomer };
}
...
async function processReminders({ dryRun = false, triggeredBy = 'manual' } = {}) {
  ...
  // Source + test-mode resolution. crmRepository owns the env interpretation so
  // the engine and repository never disagree about which table is in use.
  const source = crmRepository.getSource();
  const testMode = crmRepository.isTestMode();
  // Safety: test data must never trigger a real send.
  if (testMode && !dryRun) {
    console.warn('[engine] REMINDER_TEST_MODE=true forces dry-run — no emails will be sent');
    dryRun = true;
  }
  // Postgres dry-run = dry-run against the postgres lead source (production
  // reminder_leads, or test_leads when REMINDER_TEST_MODE=true). Gmail is never
  // required in dry-run: no token refresh, no lock check, no credential alert,
  // and lib/gmailSender.js is never imported. This keeps dry-run fully
  // independent of Gmail credentials (which may not be configured yet) and
  // prevents a dry-run from accidentally tripping the credential lock that
  // would block the first real send. The base44 dry-run path keeps its
  // existing Gmail health probe (unchanged).
  const postgresDryRun = dryRun && source === 'postgres';
  const syntheticTest = postgresDryRun && testMode;

  // Global Gmail credential lock — if set, do not send anything.
  // Skipped on every postgres dry-run (Gmail is not involved there).
  if (!postgresDryRun) {
  ...
  if (dryRun) {
    let gmailStatus;
    if (postgresDryRun) {
      // Postgres dry-run: no Gmail config, no auth attempt, no lock, no alert.
      // recordDryRun() resets the gmail_* fields to neutral ('not_attempted').
      gmailStatus = 'not_attempted';
    } else {
      const gmail = require('./gmailSender'); // lazy: base44 dry-run path only
      let gmailOk = true;
      ...
      gmailStatus = gmailOk ? 'ok' : 'credentials_invalid';
    }
    ...
    await health.recordDryRun(stats, dur, gmailStatus);
    console.log(`[engine] ══ DRY RUN END ══ scanned=${stats.scanned} eligible=${stats.eligible} gmail=${gmailStatus} dur=${dur}ms`);
    return { ok: true, dryRun: true, gmailStatus, stats, durationMs: dur, triggeredBy };
  }

  // Real run: refresh Gmail token.
  const gmail = require('./gmailSender'); // lazy: real-send path only
  let gmailToken;
  try {
  ...
  for (const lead of leads) {
    ...
    for (const w of wins) {
      ...
      const sendRes = await sendWindowEmails(lead, w, appt, gmailToken, key);
      ...
      // 4. Mark claim based on actual delivery outcome.
      if (sendRes.messageIds.length > 0) {
        // At least one email reached a recipient — mark sent with Gmail message IDs.
        await markSent(claim.id, sendRes.messageIds);
        // 5. Write REMINDER_SENT Activity to Railway activities table (best-effort).
        try {
          const { query } = require('./db/client');
          await query(
            'INSERT INTO activities (lead_id, type, content, author, source, metadata) VALUES ($1, $2, $3, $4, $5, $6)',
            [
              lead.id,
              'note',
              'Reminder sent: ' + (key || 'appointment'),
              'system',
              'manual',
              JSON.stringify({ reminder_key: key, sent_at: new Date().toISOString(), customer_skipped_no_email: !!sendRes.customerSkippedNoEmail }),
            ]
          );
        } catch (actErr) {
          console.warn('[engine] Activity write failed (non-blocking):', actErr.message);
        }
        await health.recordReminderSent(lead.id, w.key);
        stats.sent++;
        console.log(`[engine] ✓ ${w.key}${w.isCatchUp ? ' (catch-up)' : ''} for ${clientFullName} (${lead.id}) customer_skipped=${!!sendRes.customerSkippedNoEmail}`);
      } else {
        // Zero recipients reached (no customer email + no staff window, or customer opted out + no staff).
        // Mark skipped_no_recipient — terminal state, prevents re-claim hammering without false "sent".
        await markSkippedNoRecipient(claim.id);
        stats.skipped++;
        console.log(`[engine] ⊘ ${w.key}${w.isCatchUp ? ' (catch-up)' : ''} skipped_no_recipient for ${clientFullName} (${lead.id})`);
      }
    }
  }
