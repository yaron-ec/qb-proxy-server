...
async function processReminders({ dryRun = false, triggeredBy = 'manual' } = {}) {
  ...
  // Postgres synthetic test path: REMINDER_DRY_RUN=true AND REMINDER_SOURCE=postgres.
  ...
  const postgresTest = dryRun && (process.env.REMINDER_SOURCE || 'base44').toLowerCase() === 'postgres';

  // Global Gmail credential lock — if set, do not send anything.
  // Skipped on the Postgres test path (Gmail is not involved there).
  if (!postgresTest) {
  ...
  if (dryRun) {
    let gmailStatus;
    if (postgresTest) {
      // Synthetic Postgres path: no Gmail config, no auth attempt, no lock.
