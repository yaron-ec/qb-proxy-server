...
async function processReminders({ dryRun = false, triggeredBy = 'manual' } = {}) {
  ...
  // Server-authoritative transport gate FIRST — when Base44 owns this flow
  // (default) return skipped WITHOUT touching the database, so the worker needs
  // no DB to honor the gate. The Base44 automation remains the sole sender; this
  // is a hard guarantee, not just the dry-run flag.
  const scheduledTransport = transport.flowTransport('SCHEDULED_REMINDER');
  transport.logDecision('SCHEDULED_REMINDER', scheduledTransport, null, { dryRun, triggeredBy });
  if (scheduledTransport === 'base44') {
    console.log('[engine] ══ SKIP: EMAIL_SCHEDULED_REMINDER_TRANSPORT=base44 (Base44 automation is the active sender) ══');
    return { ok: true, skipped: true, reason: 'scheduled_transport_base44', durationMs: Date.now() - startedAt, dryRun, stats };
  }
