...
const actionTokenStore = require('./actionTokenStore');
const repDir = require('./repDirectory');

// Public base URL for the /r/* customer-action pages (opaque-token links in emails).
const ACTION_PUBLIC_URL = (process.env.REMINDER_PUBLIC_URL || '').replace(/\/$/, '');
...
async function sendWindowEmails(lead, win, appt, gmailToken, reminderKey) {
  ...
  if (sendCustomer) {
    ...
    let actionUrls = null;
    if (ACTION_PUBLIC_URL) {
      const repEmail = repDir.repEmailForLead(lead);
      const fp = actionTokenStore.fingerprint({ leadId: lead.id, date: appt.date, time: appt.time, type: appt.type, repEmail });
      const snapshot = {
        clientFirstName: lead.first_name || '', clientLastName: lead.last_name || '',
        apptDate: appt.date, apptTime: appt.time, apptType: appt.type,
        address: [lead.property_address, lead.city].filter(Boolean).join(', '),
        projectType: lead.project_type || '',
        repName: lead.assigned_rep_name || ownerDisplayName || lead.assigned_rep || 'EC Construction Group',
        repEmail: ownerEmail,
        repPhone: lead.assigned_rep_phone || repDir.OFFICE_PHONE,
        officePhone: repDir.OFFICE_PHONE, officeEmail: repDir.OFFICE_EMAIL,
      };
      const issued = await Promise.all([
        actionTokenStore.issueToken(db, { leadId: lead.id, appointmentFingerprint: fp, actionType: 'confirm', snapshot }),
        actionTokenStore.issueToken(db, { leadId: lead.id, appointmentFingerprint: fp, actionType: 'reschedule', snapshot }),
        actionTokenStore.issueToken(db, { leadId: lead.id, appointmentFingerprint: fp, actionType: 'contact', snapshot }),
      ]);
      actionUrls = {
        confirmUrl: `${ACTION_PUBLIC_URL}/r/confirm/${issued[0].raw}`,
        rescheduleUrl: `${ACTION_PUBLIC_URL}/r/reschedule/${issued[1].raw}`,
        contactUrl: `${ACTION_PUBLIC_URL}/r/contact/${issued[2].raw}`,
      };
    }
    const body = isPhoneCall
      ? emails.clientPhoneCallEmail({ firstName: lead.first_name || 'there', date: dateFormatted, time: timeFormatted, phone: lead.phone || 'N/A', projectType: lead.project_type || 'your project', address, ownerName: ownerDisplayName, label, isCatchUp, ...(actionUrls || {}) })
      : emails.clientMeetingEmail({ firstName: lead.first_name || 'there', date: dateFormatted, time: timeFormatted, address, projectType: lead.project_type || 'your project', ownerName: ownerDisplayName, label, isCatchUp, ...(actionUrls || {}) });
    try {
