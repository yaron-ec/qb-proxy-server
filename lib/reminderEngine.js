...
async function markSkippedNoRecipient(claimId) {
  await db.query(
    `UPDATE reminder_claims
     SET status = 'skipped_no_recipient', sent_at = NOW(), gmail_message_ids = '[]'::jsonb, last_error = NULL, last_error_type = NULL
     WHERE id = $1`,
    [claimId]
  );
}
...
async function processReminders({ dryRun = false, triggeredBy = 'manual' } = {}) {
  ...
  for (const lead of leads) {
    ...
    for (const w of wins) {
      ...
      // 4. Mark claim sent or skipped based on actual email delivery.
      if (sendRes.messageIds.length > 0) {
        ...
        // No Base44 — Railway is the sole production runtime.
        try {
          ...
          await query(
            ...
            [
              ...
              'system',
              'manual',
              JSON.stringify({ reminder_key: key, sent_at: new Date().toISOString() }),
            ]
          );
        } catch (actErr) {
        ...
        // No deliverable recipient this window (customer email null + notifyStaff=false).
        // Mark skipped_no_recipient to distinguish from actual email delivery.
        await markSkippedNoRecipient(claim.id);
        stats.skipped++;
        console.log(`[engine] ⊘ ${w.key} for ${clientFullName} (${lead.id}) — skipped_no_recipient`);
      }
    }
  }
