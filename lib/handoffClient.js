...
async function getValidToken() {
  ...
  try {
    ...
    const { rows } = await db.query('SELECT * FROM app_settings WHERE key = $1', ['handoff_bearer_token']);
...
async function fetchGraphQL(token, query, variables) {
  const response = await fetch(HANDOFF_API, {
    method: 'POST',
    headers: Object.assign(authHeaders(token), { 'User-Agent': 'EC-CRM-Railway/1.0' }),
