...
const { appointmentLocalFields } = require('./booking/appointmentView');

/**
 * Canonical appointment fields for a lead: read from the lead's active
 * appointments row (the ONLY source of truth for the appointment). Uses the
 * caller's client so it sees the same transaction's writes. Falls back to
 * appointment_* already on the row only for callers that pass a non-DB row
 * (e.g. legacy projections) and have no appointments table access.
 */
async function canonicalAppointmentFields(db, leadRow) {
  if (leadRow.id) {
    ...
    if (r && Array.isArray(r.rows)) {
      if (r.rows[0]) return appointmentLocalFields(r.rows[0]);
      return { appointment_date: null, appointment_time: null, appointment_type: null };
    }
  }
  return {
    appointment_date: leadRow.appointment_date || null,
    appointment_time: leadRow.appointment_time || null,
    appointment_type: leadRow.appointment_type || null,
  };
}
...
async function syncLeadToReminders(db, leadRow) {
  ...
  const appt = await canonicalAppointmentFields(db, leadRow);
  const hasDates = leadRow.follow_up_date || appt.appointment_date;
