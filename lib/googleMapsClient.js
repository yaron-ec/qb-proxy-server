...
function normalizeAddress(rawAddress, rawCity, rawState) {
  ...
  // Also try to extract ZIP — but ONLY after a 2-letter state code to avoid
  // matching street numbers (e.g., "14572 Fountain Brook" must NOT extract "14572" as ZIP).
  // Matches "CA 92880" or "CA 92880-1234" but NOT "14572 Fountain Brook".
  if (!zip) {
    const zipMatch = s.match(/[A-Z]{2}\s+(\d{5}(-\d{4})?)\b/i);
    if (zipMatch) zip = zipMatch[1];
  }
  ...
  if (apiKey) {
    try {
      ...
      if (data.status === 'OK' && data.results?.[0]) {
        ...
        // Parse address_components into structured fields for reconciliation
        const comps = r.address_components || [];
        const getComp = (type) => comps.find(c => c.types?.includes(type));
        const streetNumber = getComp('street_number')?.long_name || '';
        const route = getComp('route')?.short_name || getComp('route')?.long_name || '';
        const street = [streetNumber, route].filter(Boolean).join(' ');
        const city = getComp('locality')?.long_name || getComp('administrative_area_level_3')?.long_name || getComp('administrative_area_level_2')?.long_name || '';
        const state = getComp('administrative_area_level_1')?.short_name || '';
        const zip = getComp('postal_code')?.long_name || '';
        // High-confidence = exact match (not partial) and has street + city + state + zip
        const isHighConfidence = !r.partial_match && street && city && state && zip;
        return {
