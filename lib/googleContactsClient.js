 ...
 */
async function createOrUpdateContact(lead, subEmail) {
  const token = await getAccessToken(subEmail);

  // Search for existing contact
  const existing = await findContact(token, lead.email, lead.phone);
  ...
  if (existing && existing.resourceName) {
    ...
    const updateFields = ['names', 'emailAddresses', 'phoneNumbers', 'addresses'].join(',');
    const res = await fetch(
      `${PEOPLE_BASE}/${existing.resourceName}:updateContact?updateFields=${updateFields}`,
...
module.exports = { getAccessToken, createOrUpdateContact, findContact };