...
// Last-10-digits comparison — the same convention lib/captureValidation.js
// uses for lead duplicate-detection. Google Contacts numbers a human typed
// in are commonly stored as bare 10-digit US numbers, while CRM leads are
// normalized to +1XXXXXXXXXX (routes/leads.js#normalizePhone). A previous
// `googleStoredDigits.includes(incomingDigits)` check only matched when the
// stored contact had >= as many digits as the incoming, country-code-
// prefixed number — the common 10-digit-stored-contact case silently never
// matched, causing findContact() to miss a genuine existing contact and let
// createOrUpdateContact() create a duplicate instead of updating it.
  ...
  for (const r of results) {
    ...
    if (phone) {
      ...
      for (const p of phones) {
        if (p.value && target && last10Digits(p.value) === target) return person;
      }
    }
  }
  return null;
