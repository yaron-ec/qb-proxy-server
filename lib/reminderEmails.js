...
/**
 * EC Construction Group reminder email templates.
 *
 * Ported VERBATIM (HTML + CSS + copy) from the Base44 sendAppointmentReminder
 * function so customers and staff receive byte-for-byte equivalent emails
 * from either system. Pure functions — no I/O, no Deno/Node globals beyond
 * Buffer (used nowhere here). The Gmail send path lives in gmailSender.js.
 *
 * Exports: clientMeetingEmail, clientPhoneCallEmail, repReminderEmail
...
// Three prominent action buttons rendered near the end of every customer
// reminder email. Mobile-friendly (full-width stacked). No internal IDs —
// the URLs carry only opaque signed tokens.
function ctaButtons({ confirmUrl, rescheduleUrl, contactUrl }) {
  return `
    <div class="cta-wrapper" style="text-align:center;margin:32px 0;">
      <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:${GOLD};margin-bottom:14px;">Quick Actions</div>
      <a href="${confirmUrl}" class="cta-button" style="display:block;background:${NAVY};color:#ffffff !important;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:16px 24px;border-radius:6px;margin-bottom:10px;">✓ Confirm Appointment</a>
      <a href="${rescheduleUrl}" class="cta-button" style="display:block;background:#ffffff;color:${NAVY} !important;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:16px 24px;border-radius:6px;border:2px solid ${NAVY};margin-bottom:10px;">📅 Request Reschedule</a>
      <a href="${contactUrl}" class="cta-button" style="display:block;background:#ffffff;color:${NAVY} !important;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:16px 24px;border-radius:6px;border:2px solid ${GOLD};">📞 Contact My Sales Representative</a>
    </div>`;
}

// ── Client: Meeting Reminder ───────────────────────────────────────────────

function clientMeetingEmail({ firstName, date, time, address, projectType, ownerName, label, isCatchUp, confirmUrl, rescheduleUrl, contactUrl }) {
  ...
  return buildEmail(`Appointment Reminder — ${COMPANY_NAME}`, `
    ...
    ${(confirmUrl && rescheduleUrl && contactUrl) ? ctaButtons({ confirmUrl, rescheduleUrl, contactUrl }) : ''}
...
// ── Client: Phone Call Reminder ────────────────────────────────────────────

function clientPhoneCallEmail({ firstName, date, time, phone, projectType, ownerName, label, address, isCatchUp, confirmUrl, rescheduleUrl, contactUrl }) {
  const subtitle = isCatchUp
...
// ── Staff: Internal Reminder (all windows) ─────────────────────────────────
...
module.exports = { clientMeetingEmail, clientPhoneCallEmail, repReminderEmail };