...
function validateCapturePayload(body) {
  ...
  // Appointment type: Meeting or Phone Call. Defaults to Meeting for backward compat.
  const follow_up_type = body.follow_up_type ? String(body.follow_up_type).trim() : '';
  const ALLOWED_FOLLOW_UP_TYPES = ['Meeting', 'Phone Call'];
  if (follow_up_type && !ALLOWED_FOLLOW_UP_TYPES.includes(follow_up_type)) {
