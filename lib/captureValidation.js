...
// Rep who receives unassigned inbound leads (public capture default, website
// leads). One configuration point; override with DEFAULT_INTAKE_REP.
const DEFAULT_INTAKE_REP = process.env.DEFAULT_INTAKE_REP || 'Yaron Drilevich';
const MAX_FIELD = 500;
...
function validateCapturePayload(body) {
  ...
  // ── A. Appointment / site visit (OPTIONAL) ─────────────────────────────
  // Both date and time, or neither. Becomes the canonical appointments row.
  ...
  if (appointment_date && !appointment_time) errors.push('appointment_time is required when appointment_date is set');
  if (appointment_time && !appointment_date) errors.push('appointment_date is required when appointment_time is set');
  if (appointment_date && !isRealDate(appointment_date)) errors.push('appointment_date must be a valid YYYY-MM-DD date');
  if (appointment_time && !/^([01]\d|2[0-3]):[0-5]\d$/.test(appointment_time)) errors.push('appointment_time must be HH:MM (24h)');
  cleaned.appointment_date = appointment_date || null;
  cleaned.appointment_time = appointment_time || null;

  // Appointment kind: Meeting (1h travel buffer each side) or Phone Call.
  const appointment_type = body.appointment_type ? String(body.appointment_type).trim() : '';
  if (appointment_type && !['Meeting', 'Phone Call'].includes(appointment_type)) {
    errors.push('appointment_type must be "Meeting" or "Phone Call"');
  }
  cleaned.appointment_type = appointment_type || 'Meeting';

  // ── B. Follow-Up / next update (OPTIONAL, independent of A) ────────────
  const fu = normalizeFollowUp({
    follow_up_date: body.follow_up_date, follow_up_time: body.follow_up_time,
    follow_up_type: body.follow_up_date || body.follow_up_time || body.follow_up_notes ? body.follow_up_type : null,
    follow_up_notes: body.follow_up_notes, follow_up_status: body.follow_up_status,
  });
  if (!fu.ok) errors.push(...fu.errors);
  cleaned.follow_up = fu.ok && !fu.cleared ? fu.value : null;
...
module.exports = {
  ...
  validateCapturePayload, EC_DOMAIN, DEFAULT_INTAKE_REP, MAX_FIELD, MAX_NOTES, MAX_PHOTOS,
};