...
async function recordDryRun(stats, durationMs, gmailStatus) {
  // 'not_attempted' = Postgres synthetic test path: Gmail was not checked at all,
  // so reset all Gmail-related fields to a neutral, truthful state (clears any
  // stale error/lock left by a prior base44-source dry-run). Other statuses
  // ('ok' | 'credentials_invalid') leave gmail_last_error / gmail_credentials_lock
  // untouched (set by recordGmailOk / recordGmailCredentialsInvalid respectively),
  // preserving existing base44 dry-run behavior.
  const resetGmail = gmailStatus === 'not_attempted';
  await db.query(`
  UPDATE reminder_runs SET
    ...
    reminders_skipped = $4,
    consecutive_failures = 0,
    gmail_status = $5${resetGmail ? ', gmail_last_error = NULL, gmail_credentials_lock = FALSE, gmail_consecutive_failures = 0' : ''}
  WHERE id = 1`, [1, durationMs, stats.scanned, stats.skipped, gmailStatus]);
}
