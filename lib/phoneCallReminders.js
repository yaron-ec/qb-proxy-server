...
/**
 * Phone-call reminder engine (Railway-native) — runs via Railway Cron.
 *
 * Behavior:
 ...
 *
 * Sending is controlled by REMINDER_DRY_RUN (default true = no emails sent).
 */
'use strict';
...
// The call to remind about: a Phone Call APPOINTMENT (canonical appointments
// row, projected into reminder_leads.appointment_*) takes precedence;
// otherwise a timed 'Phone Call' follow-up (unchanged pre-existing behavior).
function getCallMs(lead) {
  if (lead.appointment_date && lead.appointment_time && lead.appointment_type === 'Phone Call') {
    return {
      ms: time.pacificToUtcMs(lead.appointment_date, lead.appointment_time),
  ...
  for (const lead of leads) {
    ...
    for (const win of PHONE_WINDOWS) {
      ...
      if (messageIds.length > 0) {
        ...
        stats.skipped++;
        console.log(`[phone] ⊘ ${win.key} for ${clientName} (${lead.id}) — skipped_no_recipient`);
      }
    }
  }
