...
const { fetchActiveAppointment, appointmentLocalFields } = require('./booking/appointmentView');
...
async function getLead(id) {
  ...
  try {
    ...
    const r = rows[0];
    // appointment_* come ONLY from the lead's active appointments row (the
    // `leads` table holds no copy). The follow-up is never an appointment.
    const appt = appointmentLocalFields(await fetchActiveAppointment({ query }, r.id));
