...
function partialNameMatch(qbName, leadFirst, leadLast) {
  ...
  // Fallback: also check the raw QB name (without extraction).
  // extractCustomerName assumes "Description - Customer Name" format, but some
  // QB display names use "Customer Name - Description" format. When the extraction
  // picks the wrong part, the raw-name fallback still finds the match by checking
  // if the lead's first/last name appears anywhere in the full QB display name.
  const rawNorm = normalize(qbName);
  const rawParts = rawNorm.split(/\s+/).filter(Boolean);
  if (first && first.length >= 2 && rawParts.includes(first)) return true;
  if (last && last.length >= 2 && rawParts.includes(last)) return true;
