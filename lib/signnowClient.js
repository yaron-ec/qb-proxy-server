 ...
 *
 * SignNow has NO dedicated /document/templates endpoint. Templates are
 * documents with template=true. We fetch all user documents via
 * GET /user/documents and filter for templates.
 *
 * Response: array of { id, name, page_count, roles, created }
 */
async function listTemplates() {
  const token = await getAccessToken();
  const apiBase = await getEffectiveApiBase();
  const res = await fetch(`${apiBase}/user/documents`, {
    headers: authHeaders(token),
    signal: AbortSignal.timeout(30000),
  });
  if (!res.ok) {
    const t = await res.text().catch(() => '');
    throw new Error(`SignNow list documents failed ${res.status}: ${t.substring(0, 200)}`);
  }
  const data = await res.json();
  // /user/documents returns an array of document objects
  ...
  const templates = docs
    ...
    .map(d => ({
      ...
      roles: (d.roles || []).map(r => ({ name: r.name, signing_order: r.signing_order, unique_id: r.unique_id })),
      created: d.created,
    }));
  return templates;
}
