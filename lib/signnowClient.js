...
async function verifyApiKey() {
  ...
  // The /user endpoint requires a Bearer token — the API Key is sent as
  // Bearer <API_KEY> per SignNow's authentication docs. This works for both
  // Production and Sandbox API Keys, BUT the API Key must be sent to the
  // API base URL that matches the environment it was created in:
  //   - Production API Key  → https://api.signnow.com/user
  //   - Sandbox API Key     → https://api-eval.signnow.com/user
  // A Sandbox API Key sent to the Production API returns HTTP 400 with
  // error code 1537 ("invalid_token").
  ...
  if (!res.ok) {
    ...
    // Error 1537 = "invalid_token" — the bearer token is not valid for this
    // API base URL. The most common cause is an environment mismatch: a
    // Sandbox/Development API Key sent to the Production API (or vice versa).
    if (snCode === '1537' || res.status === 400) {
      const err = new Error(
        `SignNow API key rejected (error 1537: invalid_token) by ${SIGNNOW_API_BASE}. ` +
        `This usually means the API Key was created in a different SignNow environment than ` +
        `the API base URL being called. Current API base: ${SIGNNOW_API_BASE} (environment: ${SIGNNOW_ENV}). ` +
        `If the API Key is from a Development/Sandbox application, set SIGNNOW_ENVIRONMENT=sandbox ` +
        `in Railway (or set SIGNNOW_API_BASE=https://api-eval.signnow.com). ` +
        `If the API Key is from a Production application, set SIGNNOW_ENVIRONMENT=production.`
      );
      err.code = 'SIGNNOW_AUTH_FAILED';
      err.status = 401;
      err.signnowErrorCode = snCode;
      err.apiBase = SIGNNOW_API_BASE;
      err.environment = SIGNNOW_ENV;
      throw err;
    }

    const err = new Error(
      `SignNow API key verification failed (HTTP ${res.status}) from ${SIGNNOW_API_BASE}` +
