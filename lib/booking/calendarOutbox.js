...
function buildOperation(appointment, lead, ownerEmail, kind) {
  ...
  if (kind === 'main') {
    ...
    // Kind comes from the appointment's own reserved busy_range (no travel
    // buffer = Phone Call) — never from the lead's follow-up fields.
    const isPhoneCall = isPhoneCallAppointment(appointment);
    ...
    const body = {
      ...
      // ec_appointment_kind: Phone Calls never block availability/conflict
      // detection (they're a reminder/activity, not an Appointment/Site
      // Visit) — googleAvailability.js#isExcluded reads this to fully
      // exclude a Phone Call's own Google event from busy-window
      // computation, the same way ec_kind:'travel' is already excluded.
      extendedProperties: { private: {
        ec_appointment_id: appointment.id, ec_version: versionStr, ec_kind: 'main', ec_slot: slot,
        ec_appointment_kind: isPhoneCall ? 'phone_call' : 'meeting',
      } },
    };
...
async function enqueueCancel(client, appointment, version, skipTravel) {
  if (skipTravel === undefined) skipTravel = isPhoneCallAppointment(appointment);
  const main = buildOperation(appointment, null, null, 'main');
  await enqueueRow(client, appointment, 'cancel_main', main, version);
  // A legacy in-place time edit can leave the stored Google id different from
  // the id derived from the current start time — cancel the stored one too so
  // no orphaned event survives a cancel/reschedule.
  if (appointment.google_event_id && appointment.google_event_id !== main.googleEventId) {
    await enqueueRow(client, appointment, 'cancel_main',
      { googleEventId: appointment.google_event_id, slot: 'old:' + appointment.google_event_id }, version);
  }
  if (!skipTravel) {
  ...
  if (appointment.google_travel_event_id) {
    const derivedTravel = buildOperation(appointment, null, null, 'travel').googleEventId;
    if (skipTravel || appointment.google_travel_event_id !== derivedTravel) {
      await enqueueRow(client, appointment, 'cancel_travel',
        { googleEventId: appointment.google_travel_event_id, slot: 'old:' + appointment.google_travel_event_id }, version);
    }
  }
}

async function enqueueUpdate(client, appointment, lead, ownerEmail, version, skipTravel) {
  const main = buildOperation(appointment, lead, ownerEmail, 'main');

  // Reschedule invariant: after a successful reschedule there must be exactly
  // ONE active main event at the NEW time and ZERO active events at the OLD time.
  //
  // The deterministic Google event ID includes the time (hhmm), so when the
  // time changes the ID changes. An update_main PUT to the NEW id gets 404 →
  // creates a new event (via updateEvent fallback), but the OLD event is never
  // cancelled — orphaned. Fix: when the ID changed, cancel the OLD event +
  // create the NEW event. When the ID is the same (only details changed, not
  // time), update in place.
  if (appointment.google_event_id && appointment.google_event_id !== main.googleEventId) {
    await enqueueRow(client, appointment, 'cancel_main',
      { googleEventId: appointment.google_event_id, slot: 'old:' + appointment.google_event_id }, version);
    await enqueueRow(client, appointment, 'create_main', main, version);
  } else {
    await enqueueRow(client, appointment, 'update_main', main, version);
  }

  // Travel event handling based on appointment type (skipTravel):
  //   Phone Call (skipTravel=true): cancel any existing travel event (no driving).
  //   Meeting (skipTravel=false): update/create travel event (1hr buffer).
  if (skipTravel) {
    // Phone Call: cancel any existing travel event from a prior Meeting.
    if (appointment.google_travel_event_id) {
      await enqueueRow(client, appointment, 'cancel_travel',
        { googleEventId: appointment.google_travel_event_id, slot: 'old:' + appointment.google_travel_event_id }, version);
    }
  } else {
    // Meeting: update or create the travel event.
    const travel = buildOperation(appointment, lead, ownerEmail, 'travel');
    if (appointment.google_travel_event_id && appointment.google_travel_event_id !== travel.googleEventId) {
      await enqueueRow(client, appointment, 'cancel_travel',
        { googleEventId: appointment.google_travel_event_id, slot: 'old:' + appointment.google_travel_event_id }, version);
      await enqueueRow(client, appointment, 'create_travel', travel, version);
...
async function writeGoogleEventId(pool, appointment_id, action, googleEventId) {
  ...
  await pool.query(
    `UPDATE appointments SET ${col} = $1, calendar_sync_status = 'synced',
            calendar_last_error = NULL, calendar_synced_at = NOW(), updated_at = NOW()
     WHERE id = $2`,
...
async function markFailed(pool, row, err) {
  ...
  // Surface the failure on the appointment so the CRM never shows an endless
  // "Syncing…": 'retrying' while attempts remain, 'failed' once dead-lettered.
  // Only create/update rows reflect sync state (a cancel of a removed
  // appointment has nothing left to show). Values are within the
  // appointments.calendar_sync_status CHECK ('pending','synced','retrying','failed').
  if (!String(row.action || '').startsWith('cancel')) {
    try {
      await pool.query(
        `UPDATE appointments SET calendar_sync_status = $1, calendar_last_error = $2, updated_at = NOW()
          WHERE id = $3 AND status IN ('scheduled', 'confirmed')`,
        [dead ? 'failed' : 'retrying',
         (err && err.message ? String(err.message).substring(0, 500) : 'unknown error'),
         row.appointment_id]
      );
    } catch (e) {
      console.error('[calendar-outbox] could not record sync failure on appointment', row.appointment_id, ':', e.message);
    }
  }
}
...
// ── Reconciliation: verify synced appointments still exist in Google Calendar ──
// Bounded: processes a small batch per tick. Only checks active (scheduled/confirmed)
// appointments with calendar_sync_status = 'synced' and a google_event_id.
// If Google returns 404/410 (definitively missing): marks as error + enqueues re-create
// for active FUTURE appointments only (not past, not cancelled, not Lost/DNQ leads).
// If Google returns 5xx/403/network error: SKIPS (API failure is NOT proof of deletion).
// Rate-safe: 200ms delay between Google API calls. Idempotent: re-create uses the
// existing outbox path with ON CONFLICT DO NOTHING, so repeated reconciliation never
// creates duplicate events.
async function reconcileSyncedAppointments(pool, opts) {
  opts = opts || {};
  var batchSize = opts.reconcileBatchSize || 10;
  var calId = CALENDAR_ID;
  var subEmail = (calId && calId.indexOf('@') !== -1) ? calId : YARON_EMAIL;

  var res = await pool.query(
    "SELECT a.id, a.google_event_id, a.start_at, a.busy_range, a.status, a.version, " +
  ...
  var verified = 0, missing = 0, errors = 0, repaired = 0;

  for (var i = 0; i < rows.length; i++) {
    var appt = rows[i];
    try {
      var token = await googleCalendarClient.getAccessToken(subEmail);
      var result = await googleCalendarClient.getEvent(token, calId, appt.google_event_id);

      if (result.exists) { verified++; continue; }

      if (result.reason === 'missing') {
        missing++;
        // Mark as error — stop claiming "Synced"
        await pool.query(
          "UPDATE appointments SET calendar_sync_status = 'failed', " +
          "calendar_last_error = 'Google event not found (reconciliation)', " +
          "updated_at = NOW() WHERE id = $1",
          [appt.id]
        );

        // Only repair active FUTURE appointments (not past, not Lost/DNQ)
        var isFuture = appt.start_at && new Date(appt.start_at) > new Date();
        var isLostDnq = appt.lead_status && (appt.lead_status === 'Lost' || appt.lead_status === 'DNQ');

        if (isFuture && !isLostDnq) {
          // Fetch full appointment row for buildOperation
          var fullApptRes = await pool.query('SELECT * FROM appointments WHERE id = $1', [appt.id]);
          var fullAppt = fullApptRes.rows[0];
          var newVersion = (fullAppt.version || 1) + 1;
          // Increment version for a new idempotency key + reset status to pending
          await pool.query(
            "UPDATE appointments SET version = $1, calendar_sync_status = 'pending' WHERE id = $2",
            [newVersion, appt.id]
          );
          // Build the lead object for buildOperation
          var lead = {
          ...
          var skipTravel = isPhoneCallAppointment(fullAppt);
          await enqueueCreate(pool, { ...fullAppt, version: newVersion }, lead, appt.owner_email, skipTravel);
          repaired++;
        }
      }
    } catch (e) {
      // API failure (5xx, quota, network) — do NOT mark as missing
      errors++;
      console.error('[reconcile] verification failed for appointment', appt.id, ':', e.message);
    }
    // Rate safety: 200ms delay between Google API calls
    await new Promise(function (r) { setTimeout(r, 200); });
  }

  return { checked: rows.length, verified: verified, missing: missing, errors: errors, repaired: repaired };
}
module.exports = {
  enqueueCreate, enqueueCancel, enqueueUpdate,
  claim, claimAndProcess, processRow, finalizeSynced, markFailed, reapStuck, reconcileSyncedAppointments,
