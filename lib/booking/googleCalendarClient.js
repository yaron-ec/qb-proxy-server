...
// GET a single event by ID. Returns { exists: true, event } on 200,
// { exists: false, reason: 'missing' } on 404/410.
// Throws on any other status (5xx, 403, quota) — API failure is NOT proof of deletion.
async function getEvent(token, calendarId, eventId) {
  if (!eventId) return { exists: false, reason: 'no_event_id' };
  const base = `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events`;
  const res = await fetch(`${base}/${encodeURIComponent(eventId)}`, { headers: authHeaders(token) });
  if (res.status === 200) {
    const ev = await res.json();
    return { exists: true, event: ev };
  }
  if (res.status === 404 || res.status === 410) {
    return { exists: false, reason: 'missing' };
  }
  // Any other status (5xx, 403, network) — API failure, NOT proof of deletion
  const errText = await res.text().catch(() => '');
  if (isQuota(res.status, errText)) { const e = new Error('quota_exceeded'); e.isQuota = true; throw e; }
  throw new Error(`Calendar getEvent ${res.status}: ${errText.substring(0, 300)}`);
}

module.exports = { getAccessToken, createOrUpdateEvent, updateEvent, cancelEvent, getEvent, listByExt, listEvents };