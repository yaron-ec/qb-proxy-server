...
async function getGoogleConflictWindows({ start, end, timezone }) {
  ...
  try {
    const windows = await getGoogleBusyWindows({ calendarId: CALENDAR_ID, date, timezone: tz });
...
async function getAvailability({ owner_id, date, timezone, appointment_type_id, duration_minutes, exclude_appointment_id }) {
  ...
  let googleResult;
  try {
    const googleWindows = await getGoogleBusyWindows({ calendarId: CALENDAR_ID, date, timezone: tz });
...
module.exports = {
  ...
  CalendarUnavailableError, mergeWindows, getGoogleConflictWindows, CALENDAR_ID,
};