...
const { pool, query, ensureSchema } = require('../../db/client');
const { resolveLead, lockLeadIdentity } = require('./leadResolution');
...
const { getGoogleConflictWindows } = require('./availabilityService');
...
async function resolveOwnerClient(client, input) {
  ...
  if (input.owner_email) {
    ...
    if (r.rows[0]) return r.rows[0];
    // Two first-ever submissions for the same new owner race here: the loser
    // of the owners.email UNIQUE race reads the winner's row instead of
    // failing the whole submission with a unique violation.
    const ins = await client.query(
      'INSERT INTO owners (email, display_name) VALUES ($1, $2) ON CONFLICT (email) DO NOTHING RETURNING *',
      [email, input.owner_display_name || null]
    );
    if (ins.rows[0]) return ins.rows[0];
    const again = await client.query('SELECT * FROM owners WHERE lower(email) = lower($1)', [email]);
    if (again.rows[0]) return again.rows[0];
  }
  throw new BookingError(400, 'owner_required', 'owner_id or owner_email is required');
}

// Busy window an appointment RESERVES for others (stored as busy_range):
// Meetings reserve 1h travel before and after, Phone Calls reserve only the
// call itself. This is NEVER used as the candidate window for the conflict
// check itself (see assertSlotFree) — only for what gets stored.
...
// Owner-schedule lock + overlap check (same tx as the write). Matches the
// availability DISPLAY's single-buffer rule exactly (lib/booking/slotBlocking):
// the candidate is checked as its ACTUAL, UNBUFFERED meeting window
// [candidateStart, candidateEnd] against existing rows' already-buffered
// busy_range — never re-buffered here (that would double the effective gap
// and reject slots the UI correctly showed as available). An authorized
// admin override books anyway; the overlap is recorded on the new row.
async function assertSlotFree(client, ownerId, candidateStart, candidateEnd, excludeId, overrideConflict) {
  try {
    return await acquireOwnerLockAndCheckConflict(client, ownerId, candidateStart, candidateEnd, excludeId, !!overrideConflict);
  } catch (e) {
...
// Pre-transaction Google Calendar conflict check (network I/O — must never
// run inside a DB transaction, see CLAUDE.md). Checks the candidate's ACTUAL
// meeting window against real Google Calendar events, same buffer rule as
// the CRM check, so a slot already taken by a manually-added Google event
// (no CRM appointment row) is rejected the same way — matching the
// availability display, which already merges CRM + Google. If Google can't
// be reached, this resolves to no conflict (non-blocking): the availability
// DISPLAY already fails closed (503) when Google is unreachable, so a user
// can never reach submit with stale/unknown Google state in the first place;
// the CRM check below remains authoritative either way.
async function assertNoGoogleConflict(candidateStart, candidateEnd, timezone, overrideConflict) {
  if (overrideConflict) return; // admin explicitly authorized booking over a known conflict
  const conflicts = await getGoogleConflictWindows({ start: candidateStart, end: candidateEnd, timezone });
  if (conflicts.length > 0) {
    throw new BookingError(409, 'slot_conflict', 'the requested slot conflicts with an existing Google Calendar event');
  }
}
...
// PostgreSQL deadlock_detected / serialization_failure: the transaction was
// rolled back in full, so re-running it is safe. Bounded, logged, and only for
// those two codes — any other failure (including every BookingError such as a
// real slot conflict or duplicate) is returned immediately.
...
async function withTxRetry(label, fn) {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      const code = e && ((e.details && e.details.pgCode) || e.code);
      if (attempt >= TX_ATTEMPTS || !RETRYABLE_PG_CODES.has(code)) throw e;
      console.warn(`[booking] ${label}: transient ${code} on attempt ${attempt}/${TX_ATTEMPTS} — retrying`);
...
async function createBooking(input) {
  ...
  // ── Resolve appointment type/duration + Google conflict pre-check ───────
  // BEFORE the DB transaction (Google read is network I/O; see CLAUDE.md —
  // never hold a transaction open across an external API call). Resolving
  // the type here (instead of inside the transaction, as before) lets the
  // Google check compute the ACTUAL candidate window up front; the result is
  // reused inside the transaction so the type is never looked up twice.
  let preResolvedType = null;
  let preResolvedDuration = null;
  if (withAppointment) {
    preResolvedType = await getType(appointment_type_id);
    if (!preResolvedType) throw new BookingError(400, 'invalid_appointment_type', 'appointment_type_id not found');
    preResolvedDuration = resolveDuration(preResolvedType, duration_override_minutes);
    const preStart = new Date(start_at);
    const preEnd = new Date(preStart.getTime() + preResolvedDuration * 60 * 1000);
    await assertNoGoogleConflict(preStart, preEnd, timezone, override_conflict);
  }
  ...
  // The whole transaction is one unit of work: a transient PostgreSQL deadlock
  // or serialization failure rolls back everything, so it is retried a bounded
  // number of times (defense in depth — the lock ORDER below is what prevents
  // intake deadlocks; see lockLeadIdentity).
  return withTxRetry('createBooking', async () => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');

      // 0. per-identity intake lock — ALWAYS the first lock this transaction
      //    takes (lock order: identity → lead rows → owner schedule → writes).
      //    Serializes only submissions that share a phone / email / external_ref
      //    / idempotency key, so a concurrent retry or same-person resubmission
      //    sees the first one's committed lead instead of duplicating it.
      await lockLeadIdentity(client, {
        email: input.email, phone: input.phone, external_ref: input.external_ref, idempotency_key,
      });
      ...
      // 4. duration (already resolved before the transaction, above)
      const duration = preResolvedDuration;
      ...
      // busyWindow() is what gets STORED (busy_range) — the conflict CHECK below
      // uses the actual, unbuffered [start, end] (see assertSlotFree's doc comment).
      const { busyStart, busyEnd } = busyWindow(start, end, skip_travel);
      const slot = await assertSlotFree(client, owner.id, start, end, null, override_conflict);
      let appt;
      try {
        const ins = await client.query(
      ...
      throw new BookingError(500, 'booking_failed', e.message, { pgCode: e.code });
    } finally {
      client.release();
    }
  });
...
async function rescheduleAppointment(appointment_id, { new_start_at, duration_override_minutes, appointment_type_id, actor, skip_travel, override_conflict, override_actor, onWrite }) {
  ...
  // ── Pre-transaction Google conflict check (network I/O — never inside the
  // transaction). A light, unlocked read just to resolve type/duration for
  // the check; the authoritative, lock-protected read happens below.
  const peek = await query('SELECT appointment_type_id, duration_override_minutes FROM appointments WHERE id = $1', [appointment_id]);
  if (peek.rows[0]) {
    const peekTypeId = appointment_type_id || peek.rows[0].appointment_type_id;
    const peekType = await getType(peekTypeId);
    if (peekType) {
      const peekDuration = resolveDuration(peekType, duration_override_minutes != null ? duration_override_minutes : peek.rows[0].duration_override_minutes);
      const peekStart = new Date(new_start_at);
      const peekEnd = new Date(peekStart.getTime() + peekDuration * 60 * 1000);
      await assertNoGoogleConflict(peekStart, peekEnd, null, override_conflict);
    }
  }
  ...
  try {
    ...
    const end = new Date(start.getTime() + duration * 60 * 1000);
    // Phone Calls (skip_travel=true): no travel buffer. Meetings: 1hr before/after.
    // busyWindow() is what gets STORED (busy_range) — the conflict CHECK uses
    // the actual, unbuffered [start, end] (see assertSlotFree's doc comment).
    const { busyStart, busyEnd } = busyWindow(start, end, skip_travel);
    const slot = await assertSlotFree(client, old.owner_id, start, end, old.id, override_conflict);
    let appt;
    try {
      const ins = await client.query(
        `INSERT INTO appointments
...
async function updateAppointment(appointment_id, patch, actor) {
  ...
  try {
    ...
    if (durationChanged) {
      ...
      // busyWindow() is what gets STORED (busy_range) — the conflict CHECK
      // uses the actual, unbuffered [start, end] (see assertSlotFree's doc
      // comment); passing the buffered window here double-buffers the check.
      const { busyStart, busyEnd } = busyWindow(start, end, isPhoneCall);
      await assertSlotFree(client, newOwnerId, start, end, appt.id, false);
...
async function createAppointmentForLead({ lead_id, start_at, skip_travel, appointment_type_id, duration_override_minutes,
  ...
  // ── Pre-transaction Google conflict check (network I/O — never inside the
  // transaction). Resolves type/duration the same way the transaction below
  // does (falling back to the default active type when none is given) — the
  // Google calendar is a single shared calendar (not owner-scoped), so no
  // lead/owner lock is needed for this check.
  let peekTypeId = appointment_type_id;
  if (!peekTypeId) {
    const t = await query("SELECT id FROM appointment_types WHERE is_active = true ORDER BY (name = 'Consultation') DESC, name LIMIT 1");
    peekTypeId = t.rows[0] && t.rows[0].id;
  }
  const peekType = peekTypeId ? await getType(peekTypeId) : null;
  if (peekType) {
    const peekDuration = resolveDuration(peekType, duration_override_minutes);
    const peekStart = new Date(start_at);
    const peekEnd = new Date(peekStart.getTime() + peekDuration * 60 * 1000);
    await assertNoGoogleConflict(peekStart, peekEnd, null, override_conflict);
  }
  ...
  try {
    ...
    // the actual, unbuffered [start, end] (see assertSlotFree's doc comment).
    const { busyStart, busyEnd } = busyWindow(start, end, skip_travel);
    const slot = await assertSlotFree(client, lead.owner_id, start, end, null, override_conflict);
    const ins = await client.query(
      `INSERT INTO appointments
         (lead_id, owner_id, appointment_type_id, start_at, end_at,
          duration_override_minutes, timezone, busy_range, status, idempotency_key, calendar_sync_status,
