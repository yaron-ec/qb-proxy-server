...
const projectionService = require('../projection/projectionService');
...
async function createBooking(input) {
  ...
  try {
    ...
    // 8. calendar outbox (Phase 2) — enqueued in the SAME tx; no Google API here.
    ...
    // 9. Base44 projection (Phase 3A) — one aggregate revision for the whole
    // booking (new lead + first appointment). bookingService expresses business
    // intent only; projectionService owns revision/key/outbox. Skips base44-origin.
    await projectionService.recordLeadAggregateChange(
      client, leadId,
      res.action === 'create' ? 'lead_created' : 'appointment_created'
    );
...
async function cancelAppointment(appointment_id, actor) {
  ...
  try {
    ...
    // Calendar outbox (Phase 2): cancel main + travel for this slot, same tx.
    ...
    // Base44 projection (Phase 3A): the active appointment set changed.
    await projectionService.recordLeadAggregateChange(client, cancelledAppt.lead_id, 'appointment_cancelled');
...
async function rescheduleAppointment(appointment_id, { new_start_at, duration_override_minutes, appointment_type_id, actor }) {
  ...
  try {
    ...
    // Calendar outbox (Phase 2): create main + travel for the new slot, same tx.
    ...
    // Base44 projection (Phase 3A): one aggregate revision for the reschedule
    // (old cancelled + new created in this tx).
    await projectionService.recordLeadAggregateChange(client, old.lead_id, 'appointment_rescheduled');
...
async function updateAppointment(appointment_id, patch, actor) {
  ...
  try {
    ...
    // Calendar outbox (Phase 2): refresh the Google event when event-relevant
    ...
    // Base44 projection (Phase 3A): bump ONLY for Base44-visible aggregate
    // changes — owner (-> assigned_rep) or a status transition crossing the
    // active boundary (active = scheduled/confirmed). appointment_type and
    // duration are NOT projected to Base44, so they do NOT allocate a revision.
    // projectionService owns the visibility rule; bookingService expresses intent.
    if (projectionService.isBase44VisibleUpdateChange(changes, appt.status)) {
      const reason = changes.owner ? 'owner_changed' : 'appointment_status_changed';
      await projectionService.recordLeadAggregateChange(client, appt.lead_id, reason);
    }
    await client.query('COMMIT');
