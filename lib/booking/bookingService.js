...
async function createBooking(input) {
  ...
  try {
    ...
    // Phone Calls (skip_travel=true): no 1hr travel buffer — busy range = appointment only.
    // Meetings (skip_travel=false): 1hr before + 1hr after for driving buffer.
    const busyStart = skip_travel ? start : new Date(start.getTime() - 60 * 60 * 1000);
    const busyEnd = skip_travel ? end : new Date(end.getTime() + 60 * 60 * 1000);
    // Shared conflict detection: advisory lock + conflict check (same as leads.js)
    await acquireOwnerLockAndCheckConflict(client, owner.id, busyStart, busyEnd, null, !!override_conflict);
    let appt;
    try {
      const ins = await client.query(
...
async function cancelAppointment(appointment_id, actor) {
  ...
  try {
    ...
    const cancelLead = (await client.query('SELECT * FROM leads WHERE id = $1', [cancelledAppt.lead_id])).rows[0];
    const cancelSkipTravel = !!(cancelLead && cancelLead.follow_up_type === 'Phone Call');
    await calendarOutbox.enqueueCancel(client, cancelledAppt, cancelledAppt.version, cancelSkipTravel);
...
async function rescheduleAppointment(appointment_id, { new_start_at, duration_override_minutes, appointment_type_id, actor, skip_travel, override_conflict, override_actor }) {
  await ready();
  ...
  try {
    ...
    const rescheduleLead = (await client.query('SELECT * FROM leads WHERE id = $1', [old.lead_id])).rows[0];
    const rescheduleSkipTravel = !!(rescheduleLead && rescheduleLead.follow_up_type === 'Phone Call');
    await calendarOutbox.enqueueCancel(client, oldRow, oldRow.version, rescheduleSkipTravel);
    ...
    const busyEnd = skip_travel ? end : new Date(end.getTime() + 60 * 60 * 1000);
    // Shared conflict detection: advisory lock + conflict check (same as leads.js)
    // Exclude the old appointment (being replaced) from the conflict check.
    await acquireOwnerLockAndCheckConflict(client, old.owner_id, busyStart, busyEnd, old.id, !!override_conflict);
    let appt;
    try {
      const ins = await client.query(
        `INSERT INTO appointments
    ...
    await calendarOutbox.enqueueCreate(client, appt, rescheduleLead, newOwner.email, !!skip_travel);
...
async function updateAppointment(appointment_id, patch, actor) {
  ...
  try {
    ...
    // Fetch lead to determine Phone Call vs Meeting (skip_travel) for busy_range.
    const leadForAppt = (await client.query('SELECT * FROM leads WHERE id = $1', [appt.lead_id])).rows[0];
    const isPhoneCall = !!(leadForAppt && leadForAppt.follow_up_type === 'Phone Call');
    ...
    if (durationChanged) {
      ...
      // Phone Calls (isPhoneCall=true): no travel buffer. Meetings: 1hr before/after.
      const busyStart = isPhoneCall ? start : new Date(start.getTime() - 60 * 60 * 1000);
      const busyEnd = isPhoneCall ? end : new Date(end.getTime() + 60 * 60 * 1000);
