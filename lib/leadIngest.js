...
function validateAndNormalizeLead(input) {
  ...
  const lead = {
    ...
    appointment_type: appointmentDate ? (str(input.appointment_type) || 'Meeting') : null,
...
const UPSERT_SQL = `
  INSERT INTO reminder_leads
    (id, first_name, last_name, email, phone, property_address, city, project_type,
     ...
     budget_range, notes, customer_reminders_disabled, crm_created_date, appointment_type)
...
async function upsertLead(db, lead) {
  const { rows } = await db.query(UPSERT_SQL, [
    ...
    lead.crm_created_date, lead.appointment_type || null,
  ]);
