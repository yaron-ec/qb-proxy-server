...
async function resolveGmailCreds() {
  try {
    ...
    if (cred && cred.payload && cred.payload.refresh_token && cred.payload.client_id && cred.payload.client_secret) {
      return {
        clientId: cred.payload.client_id,
        clientSecret: cred.payload.client_secret,
        refreshToken: cred.payload.refresh_token,
...
function buildMime({ to, cc, subject, htmlBody, replyTo, fromName, fromAddress, headers, attachments }) {
  ...
  const headerLines = [`From: ${from}`, `To: ${Array.isArray(to) ? to.join(', ') : to}`];
