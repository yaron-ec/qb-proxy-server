...
function tokensToArgs(environment, tokens) {
  ...
  return {
    ...
    payload: {
      ...
      // Google's granted-scope list (space-separated) from the token
      // response — never the token itself — so /status can truthfully
      // report send-only vs read-available. Not present on credentials
      // saved before this field existed (see handleStatus's scope_recorded).
