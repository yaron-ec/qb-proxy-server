...
// and the R2 upload path. Writes the CURRENT CRM HandoffEstimate schema fields so
// records are consumable by the CRM UI. Never touches the global batch SyncCursor.
  ...
  register: function (app, deps) {
    ...
    app.post('/sync/lead-estimates', requireProxySecret, async (req, res) => {
      ...
      try {
        ...
        if (!b44.isConfigured()) return res.status(503).json({ ok: false, error: 'Base44 is not configured for sync' });
        let lead = null;
        try { const _all = await b44.filter('Lead', {}); lead = (_all || []).find(function (l) { return l.id === leadId || l._id === leadId; }) || null; } catch (e) { lead = null; }
        if (!lead || (!lead.id && !lead._id)) {
          console.log('[scoped:' + reqId + '] lead-estimates 404 leadId=' + leadId);
          return res.status(404).json({ ok: false, error: 'Lead not found' });
        }
        const leadPk = lead.id || lead._id;
        const estimates = await getAllQuickBooksEstimates();
        const existing = await b44.filter('HandoffEstimate', { lead_id: leadPk });
        ...
        for (const estimate of estimates) {
          ...
          const payload = {
            ...
            qb_customer_id: (estimate.CustomerRef && estimate.CustomerRef.value) || null,
          ...
          if (existingRec) {
            ...
            else { await b44.update('HandoffEstimate', existingRec.id || existingRec._id, payload); updated += 1; }
          } else {
            const createPayload = Object.assign({}, payload, { pdf_status: 'pending', pdf_retry_count: 0 });
            await b44.create('HandoffEstimate', createPayload);
            created += 1;
          }
        }
    ...
    app.post('/sync/diagnose-lead', requireProxySecret, async (req, res) => {
      ...
      try {
        const leadId = (req.body || {}).leadId;
        if (!leadId || typeof leadId !== 'string') return res.status(400).json({ ok: false, error: 'leadId is required' });
        if (!b44.isConfigured()) return res.status(503).json({ ok: false, error: 'Base44 is not configured' });
        let lead = null, loadErr = null, loadedCount = null;
        try { const _all = await b44.filter('Lead', {}); loadedCount = (_all || []).length; lead = (_all || []).find(function (l) { return l.id === leadId || l._id === leadId; }) || null; } catch (e) { loadErr = safeMsg(e); }
        if (!lead || (!lead.id && !lead._id)) {
          console.log('[scoped:' + reqId + '] diagnose-lead 404 leadId=' + leadId + ' loaded=' + loadedCount + ' err=' + loadErr);
          return res.status(404).json({ ok: false, error: 'Lead not found (scoped v4)', leadsLoaded: loadedCount, loadError: loadErr });
        }
        const leadMatchData = {
        ...
        const dur = Date.now() - t0;
        console.log('[scoped:' + reqId + '] diagnose-lead lead=' + (lead.id || lead._id) + ' candidates=' + candidates.length + ' rule=' + matchingRule + ' estimates=' + estimateList.length + ' dur=' + dur + 'ms (read-only)');
        return res.json({ success: true, leadId: lead.id || lead._id, leadMatchData: leadMatchData, customerCandidates: candidates, selectedCustomer: selectedCustomer, matchingRule: matchingRule, estimates: estimateList, warnings: warnings });
      } catch (e) {
    ...
    app.post('/sync/estimate-pdf', requireProxySecret, async (req, res) => {
      ...
      try {
        ...
        let rec = null;
        if (crmEstimateId) {
          try { const _all = await b44.filter('HandoffEstimate', {}); rec = (_all || []).find(function (r) { return r.id === crmEstimateId || r._id === crmEstimateId; }) || null; } catch (e) { rec = null; }
          if (!rec || (!rec.id && !rec._id)) {
        ...
        if (!rec && estimateId) {
          const found = await b44.filter('HandoffEstimate', { qb_estimate_id: String(estimateId) });
        ...
        const now = new Date().toISOString();
        await b44.update('HandoffEstimate', recId, {
          pdf_url: fileUrl,
