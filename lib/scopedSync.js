...
function detectRule(customer, lead) {
  ...
  const qbCustomerId = String(customer.Id || customer.id || customer.value || '').trim();
  if (qbCustomerId && lead.qb_customer_id && String(lead.qb_customer_id).trim() === qbCustomerId) return 'qb_customer_id';
