...
/**
 * Delete an object from R2/S3. Best-effort: returns { success, error }.
 * Preserves the storage_key in the DB even on failure (retry-ready).
 * @param {string} key — R2 object key (uploads/YYYY/MM/...)
...
async function deleteObject(key) {
  ...
  try {
    ...
    // Non-fatal: the DB row keeps storage_key for a later retry.
    console.warn("[r2Client] deleteObject failed for key " + key + ":", e.message);
    return { success: false, error: e.message };
  }
}

module.exports = { s3Client, activeBucket, activePublicUrl, isConfigured, uploadBuffer, getSignedDownloadUrl, deleteObject };