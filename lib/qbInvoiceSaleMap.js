 ...
 *
 * Field naming (audited against every consumer — see the architecture review
 * and the "financial correctness" workstream that added the two fields below):
 *   - `total`, `invoiced`, `paid` are unambiguous and always correct.
 *   - `balance` has ALWAYS been computed here as `total - paid` — i.e. "how
 *     much is left to collect on the whole project" — NOT `invoiced - paid`
 *     ("unpaid portion of what's been billed so far"), despite some docs
 *     historically describing BALANCE as the latter. `Deals.jsx` and other
 *     current UI already display this exact `balance` value (as "Balance
 *     Due"/`balance_due`) and `crm-frontend/src/lib/financialCalc.js`
 *     independently recomputes it the same way — changing its VALUE now
 *     would silently change numbers already shown to users in production.
 *     `balance` is therefore left untouched for backward compatibility.
 *   - `remaining` (added) is an explicit, correctly-named alias for the
 *     existing `balance` value — new/future consumers should read
 *     `remaining` rather than the ambiguously-named `balance`.
 *   - `invoiced_unpaid` (added) is the genuinely new value: `invoiced - paid`,
 *     i.e. the unpaid portion of invoices actually issued so far (what some
 *     docs call BALANCE). This value was never computed or exposed anywhere
 *     before — it fills a real gap (e.g. a true accounts-receivable /
 *     collections view) without touching any existing field.
 */
function computeSaleFinancials(saleTotal, invoices) {
  ...
  const invoiced_unpaid = round2(Math.max(0, invoiced - paid));
  let payment_status = 'unpaid';
  if (total > 0 && paid >= total) payment_status = 'paid';
  else if (paid > 0) payment_status = 'partial';
  return { total, invoiced, paid, balance, remaining, invoiced_unpaid, payment_status };
}
