...
// google_contacts_outbox has no unique constraint on lead_id (its primary
// key is a freshly-generated id), so a plain INSERT ... ON CONFLICT DO
// NOTHING never actually dedupes — every call would insert a brand new row
// even while one for the same lead is already pending/processing. Guard
// explicitly: skip the insert if a not-yet-resolved row already exists for
// this lead. This is what makes repeated enqueue calls (e.g. a lead saved
// twice in a row) safe/idempotent rather than piling up redundant rows.
//
// Also flips the lead's own google_contact_sync_status to 'pending' so a
// caller reading the lead sees that a fresh sync is outstanding, rather
// than a stale 'synced'/'error' status left over from before this edit —
// enqueue must never look identical to "already synced".
async function enqueueContactSync(pool, leadId) {
  if (!leadId) return;
  const existing = await pool.query(
    "SELECT id FROM google_contacts_outbox WHERE lead_id = $1 AND status IN ('pending', 'processing') LIMIT 1",
    [leadId]
  );
  if (existing.rows.length === 0) {
    await pool.query(
      "INSERT INTO google_contacts_outbox (lead_id, status) VALUES ($1, 'pending')",
      [leadId]
    );
  }
  await pool.query(
    "UPDATE leads SET google_contact_sync_status = 'pending' WHERE id = $1",
    [leadId]
  );
}

// ── processContactsOutbox — claim and process a bounded batch ──
// Invariant: every pool.connect() → exactly one guaranteed client.release()
// via try/finally. ROLLBACK failure cannot leak the client.
async function processContactsOutbox(pool, opts) {
  ...
  try {
    ...
    try { await client.query('ROLLBACK'); } catch (_) { /* ROLLBACK failure must not prevent release */ }
  ...
  for (const row of rows) {
    try {
      ...
      // google_contact_synced_at is set ONLY here (never by an unrelated
      // lead edit) so reconciliation can detect drift via
      // updated_at > google_contact_synced_at — see migration 2026-38.
      await pool.query(
        "UPDATE leads SET google_contact_sync_status = 'synced', " +
        'google_contact_resource_name = $1, google_contact_sync_error = NULL, ' +
        'google_contact_synced_at = NOW(), updated_at = NOW() WHERE id = $2',
