...
if (
  /prod|production/i.test(TEST_DB) &&
  !/test|staging|dev|dry|sandbox|nurturing/i.test(TEST_DB)
) {
...
function preview(s, n = 80) {
  ...
  return c.length > n ? c.slice(0, n) + 'â€¦' : c;
}
...
async function run() {
  ...
  async function expectFail(sp, fn) {
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i++) {
      ...
      try {
        ...
        report.migration.statements.push({
          index: i + 1,
          ok: true,
    ...
    const admin = {
      role: 'admin',
      email: 'admin@ecconstructiongroup.com',
      id: 'admin-1',
    };

    const rep = {
      role: 'sales_rep',
      email: 'yaron@ecconstructiongroup.com',
      full_name: 'Yaron Drilevich',
      id: 'rep-1',
    };

    const manager = {
      role: 'manager',
      email: 'mgr@ecconstructiongroup.com',
      id: 'mgr-1',
    };

    const office = {
      role: 'office',
      email: 'off@ecconstructiongroup.com',
      id: 'off-1',
    };

    async function seedLead(externalRef) {
      const r = await client.query(
        `INSERT INTO leads (id, external_ref)
    ...
    await withTest('A', async () => {
      ...
      report.tests.A = {
        scenario:
          'Base44 Deal legacy lead id resolves through leads.external_ref',
        planStatus: plan.status,
        railwayLeadId: plan.railwayLeadId,
        pass:
          plan.status === 'migrated' &&
          plan.railwayLeadId === leadId &&
          !!inserted &&
          inserted.lead_id === leadId &&
          inserted.legacy_base44_id === 'b44deal-001',
      };
    });
    ...
    await withTest('B', async () => {
      ...
      report.tests.B = {
        scenario:
          'inserted Deal stores Railway Lead UUID; legacy id is metadata',
        leadIdInRow: d.lead_id,
        legacyLeadIdInRow: d.legacy_base44_lead_id,
        pass:
          d.lead_id === leadId &&
          d.lead_id !== 'b44lead-b' &&
          d.legacy_base44_lead_id === 'b44lead-b' &&
          d.legacy_base44_id === 'b44deal-b',
      };
    });
    ...
    await withTest('C', async () => {
      ...
      const count = await client.query(
        `SELECT COUNT(*)::int AS n
         FROM deals
         WHERE legacy_base44_id = $1`,
        ['b44deal-c']
      );

      report.tests.C = {
        scenario:
          'unresolved legacy Lead reported, not inserted, no invented lead',
        planStatus: plan.status,
        reason: plan.reason,
        dealCount: count.rows[0].n,
        pass:
          plan.status === 'unresolved' &&
          plan.reason ===
            'railway_lead_not_found_by_external_ref' &&
          count.rows[0].n === 0,
      };
    });
    ...
    await withTest('D', async () => {
      ...
      const e = await expectFail(
        'D_ins',
        () =>
          client.query(
            `INSERT INTO deals (lead_id, name)
             VALUES ($1,$2)`,
            [fakeUuid, 'Ghost Lead Deal']
          )
      );
      ...
      report.tests.D = {
        scenario:
          'FK rejects nonexistent Railway Lead',
        pass: fkRejected,
      };
    });
    ...
    await withTest('E', async () => {
      ...
      await client.query(
        `INSERT INTO deals
         (lead_id, name, amount, legacy_base44_id)
         VALUES ($1,$2,$3,$4)`,
        [leadId, 'Job 1', 4724, 'b44-e1']
      );
      ...
      report.tests.E = {
        scenario:
          'multiple Deals belong to the same Railway Lead',
    ...
    await withTest('F', async () => {
      ...
      const ownByLead = await client.query(
        `SELECT lead_id
         FROM deals
         WHERE legacy_base44_id = $1`,
        ['b44-dup']
      ...
      report.tests.F = {
        scenario:
          'legacy_base44_id unique; ownership is lead_id UUID not legacy id',
        dupRejected,
        ownerIsLead1:
          ownByLead.rows[0] &&
          ownByLead.rows[0].lead_id === lead1,
        pass:
          dupRejected &&
          ownByLead.rows[0].lead_id === lead1,
      };
        ...
        () =>
          client.query(
            `INSERT INTO deals
             (lead_id, name, stage)
             VALUES ($1,$2,$3)`,
            [leadId, 'X', 'Bogus Stage']
          )
      );

      const rejected =
        !!e &&
        /check constraint/i.test(e.message);

      report.tests.G = {
        scenario:
          'invalid stage rejected by CHECK',
        pass: rejected,
      };
    });
    ...
    await withTest('H', async () => {
      ...
      report.tests.H = {
        scenario:
          'invalid payment_status rejected',
        pass: rejected,
      };
    });
    await withTest('I', async () => {
      ...
      // NOW() is transaction-stable in PostgreSQL.
      // Seed an intentionally old updated_at so the BEFORE UPDATE trigger
      // must replace it with the transaction timestamp.
      const oldTs = '2000-01-01T00:00:00.000Z';

      const ins = await client.query(
        `INSERT INTO deals (lead_id, name, amount, updated_at)
         VALUES ($1,$2,$3,$4)
         RETURNING *`,
        [leadId, 'Update Me', 1000, oldTs]
      );

      const origUpdatedAt = ins.rows[0].updated_at;
      const origName = ins.rows[0].name;
      ...
      report.tests.I = {
        ...
        originalUpdatedAt: origUpdatedAt,
        updatedAtAfter: d.updated_date,
        ...
        pass:
          d.amount === 2000
          && d.name === origName
          && new Date(d.updated_date) > new Date(origUpdatedAt),
      };
    });
    await withTest('J', async () => {
      ...
      report.tests.J = {
        scenario:
          'RBAC pure checks (target business rules)',
        pass:
          model.canAccessDeal(admin, deal) &&
          model.canAccessDeal(manager, deal) &&
          !model.canAccessDeal(office, deal) &&
          model.canAccessDeal(rep, deal) &&
          !model.canAccessDeal(rep, other) &&
          model.canWriteDeal(
            rep,
            deal,
            'update'
    ...
    await withTest('K', async () => {
      ...
      await client.query(
        `INSERT INTO deals
         (lead_id, name, assigned_rep, created_by)
         VALUES ($1,$2,$3,$4)`,
        [
          leadId,
          'Yaron Deal',
      ...
      const cands =
        model.repMatchCandidates(rep);

      const scoped = await client.query(
        `SELECT *
         FROM deals
         WHERE (
           lower(assigned_rep) = ANY($1::text[])
           OR created_by = $2
         )`,
        [cands, rep.email]
      );
      ...
      report.tests.K = {
        scenario:
          'list scoping: rep sees own deals only',
        scopedCount: scoped.rows.length,
        pass:
          scoped.rows.length === 1 &&
          scoped.rows[0].assigned_rep ===
            'yaron@ecconstructiongroup.com',
      };
    });
    ...
    report.summary.rollback =
      'success â€” zero persistent changes';

    const testPasses =
      Object.values(report.tests).map(
        (t) => t.pass
      );
    ...
    report.summary.testsPassed =
      testPasses.filter(Boolean).length;

    report.summary.testsTotal =
      testPasses.length;
  } catch (e) {
    report.migration.errors.push({
      fatal: true,
