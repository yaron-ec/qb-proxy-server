...
async function getAppointmentTypeId(name, queryFn) {
  const { rows } = await queryFn('SELECT id FROM appointment_types WHERE name = $1', [name]);
  return rows[0]?.id || null;
}

async function runAppointmentMigration(queryFn = defaultQuery) {
  ...
  if (!consultationTypeId || !meetingTypeId) {
    throw new Error('Required appointment types not found. Run schema migrations first.');
  }
  ...
  const { rows } = await queryFn('SELECT COUNT(*) as cnt FROM appointments');
  console.log(`Railway appointments table now has: ${rows[0].cnt} rows`);
  return { created, updated, skipped, errors, leadNotFound, ownerNotFound, noDate, total: base44Leads.length };
}

module.exports = { runAppointmentMigration };

if (require.main === module) {
  if (!hasBase44Creds()) { console.error('[migrate-appts] WORKER_SECRET required'); process.exit(1); }
  runAppointmentMigration().then(() => process.exit(0)).catch(e => { console.error('[migrate-appts] fatal:', e); process.exit(1); });
}