...
test('validateDealPayload: create accepts valid UUID lead_id + legacy metadata', () => {
  const r = m.validateDealPayload({
    name: 'X',
    lead_id: '22222222-2222-4222-8222-222222222222',
    legacy_base44_id: 'b44d',
    legacy_base44_lead_id: 'b44l'
  }, { partial: false });

  return r.ok
    && r.cleaned.lead_id === '22222222-2222-4222-8222-222222222222'
    && r.cleaned.legacy_base44_id === 'b44d'
    && r.cleaned.legacy_base44_lead_id === 'b44l';
});
...
test('validateDealPayload: valid create accepted + numerics coerced', () => {
  ...
  return r.ok
    && r.cleaned.stage === 'Deposit Paid'
    && r.cleaned.amount === 5000
    && r.cleaned.deposit_amount === 1000;
});
...
test('validateDealPayload: partial update ignores lead_id + legacy fields (immutable)', () => {
  ...
  return r.ok
    && r.cleaned.stage === 'Job Completed'
    && r.cleaned.lead_id === undefined
    && r.cleaned.legacy_base44_id === undefined;
});
...
test('validateDealPayload: invalid payment_status / revenue_source / lead_cost_type rejected', () => {
  const r = m.validateDealPayload({
    ...
    financial_revenue_source: 'bogus',
    lead_cost_type: 'bogus'
  }, { partial: false });

  return !r.ok
    && r.errors.includes('invalid payment_status')
    && r.errors.includes('invalid financial_revenue_source')
    && r.errors.includes('invalid lead_cost_type');
});
...
test('canAccessDeal: sales_rep by assigned_rep email match (case-insensitive)', () => {
  const deal = {
    assigned_rep: 'yaron@ecconstructiongroup.com',
    created_by: 'other@x.com'
  };

  return m.canAccessDeal({
    role: 'sales_rep',
...
test('canWriteDeal: update — admin yes, manager YES, sales_rep own yes, office no', () => {
  const deal = {
    assigned_rep: 'yaron@ecconstructiongroup.com',
    created_by: 'y@x.com'
  };
...
test('canWriteDeal: delete — ADMIN ONLY', () => {
  const own = {
    assigned_rep: 'Yaron',
    created_by: 'yaron@ecconstructiongroup.com'
  };
...
test('applyDealUpdateGuard: job-owned fields never overwritten; assigned_rep applies to all', () => {
  const lead = {
    ...
    estimated_value: 999,
    project_type: 'BOGUS',
    property_address: '999 X',
    sold_date: '2099-01-01'
  };

  const deals = [{ id: 'd1' }, { id: 'd2' }];

  const r1 = m.applyDealUpdateGuard(
    lead,
    deals,
    ['estimated_value', 'project_type', 'property_address', 'sold_date']
  );

  const r2 = m.applyDealUpdateGuard(
    lead,
    deals,
    ['assigned_rep']
  );

  const noJobOwned1 = Object.keys(r1.updates)
    .every((k) => !m.JOB_OWNED_FIELDS.includes(k));

  const noJobOwned2 = Object.keys(r2.updates)
    .every((k) => !m.JOB_OWNED_FIELDS.includes(k));

  return noJobOwned1
    && r1.targets.length === 2
    && r2.updates.assigned_rep === 'Yaron'
    && noJobOwned2;
});

test('repMatchCandidates: email + full_name + derived', () => {
  const c = m.repMatchCandidates({
    email: 'YARON@ecconstructiongroup.com',
    full_name: 'Yaron Drilevich'
  });

  return c.includes('yaron@ecconstructiongroup.com')
    && c.includes('yaron drilevich');
});
...
test('planDealMigration: A — legacy lead id resolves -> migrated, lead_id = Railway UUID', () => {
  ...
  return plan.status === 'migrated'
    && plan.railwayLeadId === railwayLeadId
    && plan.dealPayload.lead_id === railwayLeadId
    && plan.dealPayload.legacy_base44_id === 'b44deal-001'
    && plan.dealPayload.legacy_base44_lead_id === 'b44lead-joann';
});
...
test('planDealMigration: B — deal payload stores Railway UUID as lead_id, not legacy id', () => {
  ...
  return plan.dealPayload.lead_id === railwayLeadId
    && plan.dealPayload.lead_id !== 'b44lead-b';
});
...
test('planDealMigration: C — unresolved legacy Lead reported, no invented lead', () => {
  ...
  return plan.status === 'unresolved'
    && plan.reason === 'railway_lead_not_found_by_external_ref'
    && plan.legacyBase44LeadId === 'b44lead-NOWHERE'
    && plan.dealPayload === undefined;
});

test('planDealMigration: D — no legacy lead_id -> unresolved', () => {
  const b44 = { id: 'b44deal-d', name: 'D' };
  const plan = m.planDealMigration(b44, null);

  return plan.status === 'unresolved'
    && plan.reason === 'no_legacy_lead_id'
    && plan.dealPayload === undefined;
});
...
test('planDealMigration: E — multiple deals same legacy lead -> same Railway UUID', () => {
  ...
  const p1 = m.planDealMigration({
    id: 'b44-1',
    lead_id: 'b44lead-shared',
    name: 'J1'
  }, railwayLeadId);

  const p2 = m.planDealMigration({
    id: 'b44-2',
    lead_id: 'b44lead-shared',
    name: 'J2'
  }, railwayLeadId);

  return p1.status === 'migrated'
    && p2.status === 'migrated'
    && p1.dealPayload.lead_id === railwayLeadId
    && p2.dealPayload.lead_id === railwayLeadId
    && p1.dealPayload.legacy_base44_id === 'b44-1'
    && p2.dealPayload.legacy_base44_id === 'b44-2';
});

test('planDealMigration: F — ownership is lead_id UUID; legacy IDs are metadata', () => {
  const railwayLeadId = '55555555-5555-4555-8555-555555555555';

  const plan = m.planDealMigration({
    id: 'b44-f',
    lead_id: 'b44lead-f',
    name: 'F'
  }, railwayLeadId);
