...
function createMockDb() {
  ...
  async function query(sql, params = []) {
    ...
    if (/INSERT INTO reminder_leads/.test(normalizedSql) && /ON CONFLICT \(id\)/.test(normalizedSql)) {
      const id = params[0];
    ...
    if (/UPDATE reminder_leads.*SET follow_up_date = NULL/.test(normalizedSql)) {
      const id = params[0];
    ...
    if (/DELETE FROM reminder_leads/.test(normalizedSql)) {
      const id = params[0];
    ...
    if (/SELECT.*FROM reminder_leads/.test(normalizedSql)) {
      const rows = Array.from(tables.reminder_leads.values());
      return { rows };
    }

    // BEGIN / COMMIT / ROLLBACK
    if (/^BEGIN/.test(normalizedSql)) { inTransaction = true; return { rows: [] }; }
    if (/^COMMIT/.test(normalizedSql)) { inTransaction = false; return { rows: [] }; }
    if (/^ROLLBACK/.test(normalizedSql)) { inTransaction = false; return { rows: [] }; }
