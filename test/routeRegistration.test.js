...
const MAIN_MARKERS = ['/health','/auth/connect','/auth/status','/auth/callback','/auth/disconnect','/company','/customers','/customers/search','/estimates','/estimates/:id/pdf','/invoices','/invoices/:id/pdf','/qb/health','/api/files','/remind','/sync/qb-estimates','/sync/qb-estimate-pdfs','/internal/email/send','/diag/base44-config','/diag/base44-gateway','/diag/sdk-test','/api/v1/auth'];
...
test('routes/emails (email API) remains mounted at /api/v1', () => { assert.ok(/require\(['"]\.\/routes\/emails['"]\)/.test(src), 'routes/emails mount removed'); });
