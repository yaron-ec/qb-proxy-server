...
// ── Test 2: Both delete handlers call cleanupLeadTextRefs within transaction ──
test('Lead delete: both /:id and /by-external call cleanupLeadTextRefs in transaction', () => {
  const src = fs.readFileSync(path.join(ROOT, 'routes', 'leads.js'), 'utf8');

  // Find both delete handlers
  const deleteIdMatch = src.match(/router\.delete\('\/:id'[\s\S]*?cleanupLeadTextRefs[\s\S]*?COMMIT/);
  const deleteExtMatch = src.match(/router\.delete\('\/by-external[\s\S]*?cleanupLeadTextRefs[\s\S]*?COMMIT/);

  assert.ok(deleteIdMatch, 'DELETE /:id must call cleanupLeadTextRefs before COMMIT');
  assert.ok(deleteExtMatch, 'DELETE /by-external must call cleanupLeadTextRefs before COMMIT');
...
test('Lead delete: migration 2026-26 fixes all blocking FK constraints', () => {
  ...
  assert.ok(/appointments.*ON DELETE CASCADE/i.test(migration), 'appointments must be CASCADE');
