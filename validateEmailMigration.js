 ...
 *     taskReminderEngine, base44TokenVerify, gmailSender, routes/gmail).
 ...
 *   - All reminder templates render locally and Base44/Railway parity holds.
 *   - Idempotency keys are deterministic.
 *   - Auth role decisions are correct for fixtures.
 *   - Transport resolution is 'base44' for every flow by default.
...
(async () => {
  ...
  const mods = [
    ...
    './lib/base44TokenVerify', './lib/gmailSender', './routes/gmail',
    './routes/emails',
  ];
  for (const m of mods) {
  ...
  section('2. Transport resolution (default base44)');
  try {
    const tc = require('./lib/transportControl');
    for (const f of tc.FLOWS) {
      const t = tc.flowTransport(f);
      if (t === 'base44') ok(`flow ${f} -> base44`);
      else bad(`flow ${f}`, `expected base44 got ${t}`);
    }
    // phone/task gates default base44 too
    if (process.env.EMAIL_PHONE_CALL_REMINDER_TRANSPORT !== 'railway') ok('phone gate base44');
    else bad('phone gate', 'railway (real sending would be enabled)');
    if (process.env.EMAIL_TASK_REMINDER_TRANSPORT !== 'railway') ok('task gate base44');
    else bad('task gate', 'railway (real sending would be enabled)');
  ...
  try {
    // ESM module: load via dynamic import
  ...
  section('6. Reminder template parity (Base44 vs Railway)');
  try {
    ...
    // subject + idempotency + activity parity
  ...
  section('7. Phone/task engine gates (no-send)');
  try {
  ...
  try {
    // gmailSender.sendEmail is the only send path. We never called it (unless
    // --refresh-gmail, which only refreshes). Assert by monkey-patching flag.
  ...
  try {
    const files = ['lib/authorization.js','lib/phoneCallReminders.js','lib/taskReminderEngine.js',
      'lib/base44TokenVerify.js','lib/emailService.js','routes/gmail.js','routes/emails.js',
