 ...
 *   invoice. Called by the CRM (Base44 qbSync or the proxy invoice flow)
 *   with the QB-created invoice id + the crm_sale_id it belongs to.
 *   Body: { qb_invoice_id, qb_doc_number, crm_sale_id, crm_lead_id,
 *           qb_customer_id, total_amt, mapping_method? }
 ...
 *   (During migration the CRM supplies the resolvers; once leads/deals live
 *   on Railway the route resolves them internally.)
 *
 * MOUNT: app.use('/api/v1/sale-invoices', require('./routes/saleInvoices')(db, auth));
 */
